...
#define AgeHCW 5 // HCW를 써야한다면 위에 Total부분 ++
...
double output; ///////// 아웃풋 항 나중에 수정해야할것
		...
		for (int type = ItypeV; type <= ItypeH; type++)  // type V, W, X, H
		{
			for (int ageSus = Age0to6; ageSus <= Age13to18; ageSus++) {
				for (int ageInf = Age0to6; ageInf <= Age13to18; ageInf++) {
					if (ageSus == ageInf) {
						beta[ageSus][ageInf][MedNO][type][IsoNO] = contagiousness[type] * b * ContactMatrix[ageSus][ageInf] * (1.0 - SchoolContactRate); // 결석으로 인해 학교 나이대 서로서로 접촉 X
					}
					else {
			...
			for (int ageSus = Age19to64; ageSus <= AgeHCW; ageSus++) { // 결석한 학생들이 care받으면서 생기는 전염성. 대상은 학생나이대 이후 전부, HCW포함
				for (int ageInf = Age0to6; ageInf <= Age13to18; ageInf++) {
